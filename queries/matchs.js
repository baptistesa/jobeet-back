...
function getUserMatch(req, res, next) {
    ...
    auth.verifyJWTToken(token)
        .then((decodedToken) => {
            ...
            db.query("SELECT matchs.id, matchs.id_user, matchs.id_recruteur, matchs.id_offre, matchs.is_valid, users.name, users.last_name, users.picture_path, users.id_entreprise FROM matchs, users WHERE matchs.id_user = ? AND matchs.id_recruteur = users.id", id_user, function (error, results, fields) {
                if (error)
                    res.status(500)
                        .json({
                            status: "ko",
...
function getRecruteurMatch(req, res, next) {
    ...
    auth.verifyJWTToken(token)
        .then((decodedToken) => {
            ...
            db.query("SELECT matchs.id, matchs.id_user, matchs.id_recruteur, matchs.id_offre, matchs.is_valid, users.name, users.last_name, users.picture_path, users.id_entreprise FROM matchs, users WHERE id_recruteur = ? AND matchs.id_user = users.id", [id_recruteur], function (error, results, fields) {
                if (error)
                    res.status(500)
                        .json({
                            status: "ko",
